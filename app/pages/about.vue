<template>
    <UContainer class="mt-2">
        <!-- top -->
        <section class="space-y-3 md:space-y-0 my-3 md:mb-6 flex flex-wrap justify-around items-center md:space-x-3">
            <!-- left card -->
            <UCard class="w-full md:w-[450px] h-56 text-center rounded-3xl nano-container">
                <UAvatar :src="data?.data.data.avatar_url" alt="Avatar" icon="i-heroicons-photo" size="3xl" />
        ...
        <section class="space-y-3 md:space-y-0 my-3 md:mb-6 flex flex-wrap justify-around items-center md:space-x-3">
            <!-- left card -->
            <UCard class="w-full md:w-64 h-56 rounded-3xl">
                <section v-for="(item, index) in mainStack" :key="index" class="flex justify-between items-center my-2">
                    <h1 class="text-md font-semibold">{{ item.name }}</h1>

                    <div class="w-24 h-5 bg-primary/20 px-[2px] rounded-lg flex items-center">
                        <span class="bg-primary h-4 rounded-lg" :style="{ width: item.value + '%' }"></span>
                    </div>
                </section>
                >
            </UCard>
            ...
            <!-- right card -->
            <UCard class="w-full h-64 md:w-[450px] md:h-56 flex justify-center items-center rounded-3xl">
                <!-- calendar of study -->
                <section class="flex justify-between items-center mb-3">
                    <section class="flex items-center border-cyan-800 border-2 rounded-full p-2">
                ...
                <!-- tags group 1 -->
                <section class="mt-3 flex flex-wrap justify-start items-center space-x-2">
                    <span v-for="(item, index) in courseTags" :key="index" class="text-xs font-semibold">#{{ item
                    }}</span>
                </section>
                <!-- tags group 2 -->
                <section class="flex flex-wrap justify-start items-center space-x-2">
                    <span v-for="(item, index) in courseTags2" :key="index" class="text-xs font-semibold">#{{ item
                    }}</span>
                </section>
            </UCard>
        </section>
        ...
        <section class="space-y-3 md:space-y-0 my-3 md:mb-6 flex flex-wrap justify-around items-center md:space-x-3">
            ...
            <UCard class="w-full md:w-[384px] h-56 rounded-3xl text-center space-x-2">
                <div class="text-start">
                    <UTooltip v-for="(item, index) in frontendStackIcon" :key="index" :text="item.name">
            ...
            <UCard class="w-full md:w-24 h-56 rounded-3xl text-center space-x-2">
                <UTooltip>
                    <div class="text-start">
                        <UTooltip v-for="(item, index) in backendStackIcon" :key="index" :text="item.name">
                            <UIcon :name="item.icon" class="w-[40px] h-[40px] mx-1" />
                        </UTooltip>
                    </div>
                </UTooltip>
            ...
            <UCard class="w-full md:w-48 h-56 rounded-3xl text-center space-x-2">
                <UTooltip>
                    <div class="text-start">
                        <UTooltip v-for="(item, index) in toolStackIcon" :key="index" :text="item.name">
                            <UIcon :name="item.icon" class="w-[40px] h-[40px] mx-1" />
                        </UTooltip>
                    </div>
                </UTooltip>
...
if (error.value) router.push('/error')
</script>
